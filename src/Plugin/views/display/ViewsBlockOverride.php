      ...
      foreach ($items as $id => $item) {
        ...
        $form['override'][$type][$id]['value'] = [
          ...
          '#states' => [
            'visible' => [
              [
                ':input[name*="[override][' . $type . '][' . $id . '][enabled]"]' => array('checked' => TRUE),
              ],
            ],
          ],
        ];
        ...
        if (isset($item->options['validate']) && 
          ...
          $entity_type = $split[1];
          $form['override'][$type][$id]['value']['#type'] = 'entity_autocomplete';
          $form['override'][$type][$id]['value']['#target_type'] = $entity_type;
          if ($value) {
            $storage = \Drupal::entityTypeManager()->getStorage($entity_type);
            $entity = $storage->load($value);
            $form['override'][$type][$id]['value']['#default_value'] = $entity;
          }
          else {
            $form['override'][$type][$id]['value']['#default_value'] = '';
          }
          if (isset($item->options['validate_options']) &&
              isset($item->options['validate_options']['bundles'])) {
            $form['override'][$type][$id]['value']['#selection_settings']['target_bundles'] = 
  ...
  public function execute() {
    ...
    if (!empty($config['contextual_filter'])) {
      foreach ($config['contextual_filter'] as $id => $values) {
        if ($values['enabled']) {
          $this->view->setArguments([$values['value']]);
        }
      }
    }
